...
export function generateFactoryCode(
    ...
    switch (node.kind) {
      ...
      default:
        if (node.kind >= ts.SyntaxKind.FirstToken && node.kind <= ts.SyntaxKind.LastToken) {
          writer
            .write('factory.createToken(ts.SyntaxKind.')
  ...
  function writeNodeTextForTypeNode(node: import('typescript').TypeNode) {
    if (node.kind >= ts.SyntaxKind.FirstKeyword && node.kind <= ts.SyntaxKind.LastKeyword) {
      writer
        .write('factory.createKeywordTypeNode(ts.SyntaxKind.')
        .write(syntaxKindToName[node.kind])
        .write(')');
    } else {
      writeNodeText(node);
        ...
        if (node.modifiers.length === 1) {
          ...
          writer.indent(() => {
            for (let i = 0; i < node.modifiers!.length; i++) {
              ...
              if (i > 0) writer.write(',').newLine();
              writer.write(
                'factory.createModifier(ts.SyntaxKind.' + syntaxKindToName[item.kind] + ')'
              );
            }
          });
        }
  ...
  function createClassStaticBlockDeclaration(
    node: import('typescript').ClassStaticBlockDeclaration
  ) {
  ...
  function createMappedTypeNode(node: import('typescript').MappedTypeNode) {
    ...
    writer.indent(() => {
      ...
      writeNodeText(node.typeParameter);
      writer.write(',').newLine();
      if (node.nameType == null) writer.write('undefined');
      else {
      ...
      writer.write(',').newLine();
      if (node.members == null) writer.write('undefined');
      else {
  ...
  function createVariableDeclaration(node: import('typescript').VariableDeclaration) {
    ...
    writer.indent(() => {
      writeNodeText(node.name);
      writer.write(',').newLine();
      if (node.exclamationToken == null) writer.write('undefined');
      else {
  ...
  function createImportDeclaration(node: import('typescript').ImportDeclaration) {
    ...
    writer.indent(() => {
      ...
      writer.write(',').newLine();
      if (node.assertClause == null) writer.write('undefined');
      else {
  ...
  function createAssertClause(node: import('typescript').AssertClause) {
    ...
    writer.indent(() => {
      writer.write('/* unknown */');
      writer.write(',').newLine();
      if (node.multiLine == null) writer.write('undefined');
      else {
  ...
  function getNodeFlagValues(value: number) {
    ...
    return getFlagValuesAsString(
      ...
      value || 0,
      'None',
      getFlagValues(ts.NodeFlags, value).filter(v => v !== ts.NodeFlags.BlockScoped)
    );
  }
